...
async function handleApi(pathname, body, res, qs) {
  ...
  if (pathname === '/api/scan/status') {
    ...
    return ok(res, {
      ...
      error: job.error,
      report: rep || null,
      // T2：GUI 也要能看到报告的出处（生成时间 + 范围），否则界面上分不出新旧
      provenance: require('./report.js').describe(rep || null),
      progress: p || null,
  ...
  if (pathname === '/api/report') {
    ...
    return ok(res, { report: rep, provenance: require('./report.js').describe(rep) });
  }

  // GET /api/reports — 报告历史（补能力矩阵 §3 第 1 项：CLI 有 `report --history`、
  // MCP 有 `section=history`，而 GUI 此前连端点都没有）。
  //   ?file=<name>  取某一份归档的完整内容（读取端透明解压）
  //   无参          返回归档清单（不逐个解压：真实整盘报告可能几十 MB，列表不值这个代价）
  if (pathname === '/api/reports') {
    const reportLib = require('./report.js');
    const want = qs.get('file');
    if (want) {
      // 只接受历史目录内的**文件名**。报告名来自 HTTP 查询串，直接用会形成
      // 目录穿越（`../` 能读到任意 .json/.gz）。basename + 父目录比对两道一起上。
      const safe = path.basename(String(want));
      const full = path.join(audit.reportsDir(), safe);
      if (safe !== String(want) || path.dirname(full) !== audit.reportsDir() ||
          !/\.json(\.gz)?$/i.test(safe)) {
        return bad(res, '非法的报告文件名', 400);
      }
      const rep = audit.readJson(full);
      if (!rep) return bad(res, '无法读取该归档：' + safe, 404);
      return ok(res, { report: rep, provenance: reportLib.describe(rep), name: safe });
    }
    return ok(res, { items: reportLib.listArchives().slice(0, 50), dir: audit.reportsDir() });
  }
  ...
  // ---- T11：管理员权限 ----
  // GUI 安装包已从 requireAdministrator 改为 asInvoker（不再每次启动弹 UAC），
  // 于是"哪些功能需要管理员"必须由**服务层**明确说出来。否则用户在这些功能上
  // 只会看到一句来自底层的晦涩失败，而不知道该怎么办。
  // 进程权限在生命周期内不会变化，所以只检测一次（spawn 一次 net session 有开销）。
  let adminCache = null;
  function isAdmin() {
    if (adminCache === null) {
      try {
        adminCache = require('child_process').spawnSync('net', ['session'], { windowsHide: true }).status === 0;
      } catch (e) { adminCache = false; }
    }
    return adminCache;
  }
  const ADMIN_HINT =
    '该功能需要管理员权限：MFT 直读、SMART 健康检查、配额分析都要读底层卷信息。' +
    '解决办法：关闭窗口，右键 disk-clean → 「以管理员身份运行」，然后重试。' +
    '（常规扫描、清理、整理、重复文件检测都不需要管理员，照常可用。）';

  // GET /api/health-check（SMART/SSD；服务端缓存 TTL 30s，?force=1 绕过）
  if (pathname === '/api/health-check') {
    if (!isAdmin()) return bad(res, ADMIN_HINT, 403);
