...
function listDrives() {
  ...
  for (let i = 65; i <= 90; i++) {
    ...
    try {
      fs.accessSync(ch + ':\\');
      // statfsSync：真实卷容量（bsize*blocks）。此前用 statSync(root).blocks 只会得到
      // 根目录自身占用的块数（几十 KB），是显示 24kb 之类错误数字的根因。
      const s = fs.statfsSync(ch + ':\\');
      ...
      out.push({ drive: ch + ':', total: total, free: free, avail: avail, used: Math.max(0, total - free) });
    } catch (e) { /* skip */ }
  }
  return out;
}
...
const SYS_RE = /\\windows\\|\\program files\\|\\program files \(x86\)\\|\\programdata\\|\\winsxs\\|\\system volume information\\|\\\$recycle\.bin\\/i;
const TEMP_RE = /(\\temp\\|\\tmp\\|\\cache\\|\\prefetch\\|\\thumbcache\\|\\iconcache\\|(^|[\\/])(temp|tmp|cache|prefetch|thumbcache|iconcache)([\\/]|$))/i;
function safeOnly(list) { return (list || []).filter(function(p) { return !SYS_RE.test(String(p).toLowerCase()) }) }
...
async function handleApi(pathname, body, res, qs) {
  ...
  if (pathname.startsWith('/api/recycle/list')) {
    ...
    for (const e of auditEntries) {
      if (e && Array.isArray(e.paths)) for (const p of e.paths) toolPaths.add(String(p).toLowerCase());
      if (e && Array.isArray(e.keys)) for (const k of e.keys) toolPaths.add(String(k).toLowerCase());
    }
    const items = all.filter(function(x) { return toolPaths.has(String(x.originalPath).toLowerCase()); });
  ...
  if (pathname === '/api/recycle/restore' && body && typeof body === 'object') {
    ...
    const auditEntries = audit.readAudit();
    const toolPaths = new Set();
    for (const e of auditEntries) if (e && Array.isArray(e.paths)) for (const p of e.paths) toolPaths.add(String(p).toLowerCase());
    const matchedKeys = new Set(all.filter(function(x){ return toolPaths.has(String(x.originalPath).toLowerCase()); }).map(function(x){ return String(x.key).toLowerCase(); }));
