...
async function execute(type, paths, report, dryRun) {
  ...
  try {
    ...
    appendAudit({
      ...
      result: succeeded ? 'ok' : 'error',
      detail: succeeded ? '' : (errText || ('executed=0 of ' + v.paths.length))
    });
    if (!succeeded) {
      return {
        ok: false,
    ...
    return {
      ok: true, executed: executed, total: v.paths.length, freedBytes: v.estBytes,
      escapedWhitelist: v.escapedWhitelist,
      note: '已移入回收站 ' + executed + '/' + v.paths.length + ' 项'
    };
  } catch (e) {
