...
// OneDrive 云同步路径：删除会同步影响云端，"移入回收站可恢复"对云不成立 —— 一律拒绝
function isCloudSyncPath(lp) {
  return lp.split(BS).some(function(s) { return s === 'onedrive' || s.indexOf('onedrive') === 0 });
}
...
function validate(type, paths, report) {
  ...
  for (const p of paths) {
    ...
    if (isCloudSyncPath(lp)) return err('拒绝清理 OneDrive 云同步文件（删除会同步影响云端）：' + p);
  }
  if (type === 'duplicates') {
