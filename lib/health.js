...
function historyFile() { return path.join(os.homedir(), '.disk-clean', 'health-history.json'); }
// 历史写入节流：同盘两次记录间隔 ≥60s（防止频繁打开健康页产生噪声趋势点）
var HISTORY_MIN_INTERVAL_MS = 60 * 1000;
function appendHistory(disks) {
  try {
    var hist = {};
    try { var raw = fs.readFileSync(historyFile(), 'utf8'); if (raw.charCodeAt(0) === 0xFEFF) raw = raw.slice(1); hist = JSON.parse(raw) || {}; } catch (e) { hist = {}; }
    var now = Date.now();
    var nowIso = new Date().toISOString();
    var changed = false;
    disks.forEach(function(d){
      var key = d.serial || d.name;
      if (!key) return;
      if (!hist[key]) hist[key] = [];
      var arr = hist[key];
      // 节流：最后一条记录不足 60s 则跳过本次写入
      if (arr.length > 0) {
        var last = Date.parse(arr[arr.length - 1].ts);
        if (!isNaN(last) && now - last < HISTORY_MIN_INTERVAL_MS) return;
      }
      arr.push({ ts: nowIso, temp: d.temp, wear: d.wear, poh: d.poh, readErrUncorr: d.readErrUncorr, writeErrUncorr: d.writeErrUncorr, readErrCorr: d.readErrCorr, writeErrCorr: d.writeErrCorr });
      if (arr.length > 100) hist[key] = arr.slice(-100);
      changed = true;
    });
    if (!changed) return hist;
...
function check() {
  ...
  var hist = appendHistory(disks);
  // Attach trend per disk (last 20 points for sparklines)
  var trend = {};
  try {
    var fullHist = hist || readHistory();
    disks.forEach(function(d){
      var key = d.serial || d.name;
      if (fullHist[key]) trend[key] = fullHist[key].slice(-20);
    });
  } catch (e) {}
  return { ok: true, disks: disks, file: outFile, trend: trend };
}
