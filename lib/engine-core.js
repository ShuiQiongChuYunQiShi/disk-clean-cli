...
const i18nDetect = require('./i18n.js').detect;
...
// 规则单源：所有规则表与阈值默认值来自 lib/rules.js
const Rules = require('./rules.js');
const { DAYS, DUP_FULL_LIMIT, DUP_HEAD, DUP_TAIL, WIDE_MAX_FILES, WIDE_MAX_BYTES, LOOSE_MAX, MAX_DEPTH, CONCURRENCY,
  EXT_CAT, EXT_JUNK, DIR_CAT_RULES, JUNK_RULES, AUTO_SKIP, USER_ZONE_SEGS, APP_ZONE_SEGS, DRIVE_ROOT_SKIP, CLOUD_SYNC_SEG,
  catOf, dirCat,
  PROG_NAME_HINTS, PROG_DIR_HINTS, PROG_EXT, ORG_CAT_MAP } = Rules;
let STALE_MS = Rules.DEFAULTS.STALE_MS;
let STALE_LARGE_MIN = Rules.DEFAULTS.STALE_LARGE_MIN;
let DUP_MIN_SIZE = Rules.DEFAULTS.DUP_MIN_SIZE;
let LOOSE_MS = Rules.DEFAULTS.LOOSE_MS;
let LOOSE_MIN = Rules.DEFAULTS.LOOSE_MIN;
...
// 扩展查重范围：扫描根浅层（目录深度 ≤2）排除程序/游戏特征与系统段 —— 数据盘无用户区时的 fallback
...
// 分类判定（dirCat / catOf）与规则表同在 lib/rules.js —— 同源，且测试可直接回归规则表。
// 这里只保留一个返回 '其他' 兜底的薄封装，供只需"目录分类"的调用点使用。
function classifyDir(segs) { return dirCat(segs) || '其他' }
...
let bySizeWide = new Map();  // size -> [{path, size, wide}] 扩展候选：扫描根浅层非程序目录
let wideCount = 0;           // 扩展候选已收集文件数（封顶防哈希阶段卡顿）
let userZoneSeen = false;    // 本次扫描是否见过用户区目录（报告 dupScan 用）
let cloudSyncSkipped = 0;    // OneDrive 云占位跳过计数（报告提示用）
let junkPaths = new Map();   // junk label -> Set<path>（一键清理用的真实路径样本）
...
// 原子写（评审 B3）：先写同目录临时文件，再 rename 覆盖。
// 直接 writeFileSync 在中途被杀会留下半截 JSON，而读取端一律把解析失败 catch 成
// "空数组"——于是回滚记录静默消失，已经移动过的文件再也回不来。
// 同目录 rename 在 NTFS 上是原子替换，读端要么看到旧的完整内容，要么看到新的完整内容。
...
async function walk(dir, segs, depth, inAppZone) {
  ...
  if (fileEntries.length > 0) {
    await pool(fileEntries, async function(en) {
      ...
      // T6：未分类的部分按扩展名累计，报告再据此输出"未分类 Top"。
      // 缺口必须能被看见，否则"其他 XX%"永远是个没人能处理的数字。
      if (cat === '其他') {
        if (!results.uncatExt) results.uncatExt = {};
        results.uncatExt[ek] = (results.uncatExt[ek] || 0) + sz;
      }
      ...
      // Always collect time buckets for age distribution (cheap, already have stat)
      ...
      if (suggestMode && !appZone) {
        const isCloudSync = segs.indexOf(CLOUD_SYNC_SEG) >= 0 || low(dir).indexOf(CLOUD_SYNC_SEG) >= 0;
        if (isCloudSync) { cloudSyncSkipped++; }
        // 去重候选：① 用户区 ≥1MB（原有）；② 扫描根浅层（深度≤2）非程序目录 —— 数据盘无用户区时的 fallback
        // OneDrive 云占位排除：哈希会触发静默下载，整体不参与查重
        let dz = null;
        if (sz >= DUP_MIN_SIZE && !isCloudSync) {
          if (isDedupZone(dir)) {
        ...
        // 陈旧大文件建议排除云同步目录（删除会同步影响云端）
        if (now - st.mtimeMs > STALE_MS && sz > 0 && !isCloudSync) staleFiles.push({ path: path.join(dir, en.name), size: sz, modified: st.mtime.toISOString().slice(0, 10) });
        if (isDedupZone(dir)) {
...
function finalize() {
  ...
  // T6（v9-2）：未分类部分按扩展名排出来。
  // 没有这一段，规则缺口只会表现为一个"其他 XX%"的大数字，没人知道该补什么；
  // 有了它，缺口自己暴露（例如"pak 296.88 GB 未分类"直接指向要补的规则）。
  const uncategorizedTop = Object.keys(results.uncatExt || {})
    .map(k => ({ ext: k, bytes: results.uncatExt[k] }))
  ...
  return {
    summary: {
      ...
      dupScan: suggestMode ? { userZoneSeen: userZoneSeen, wideCandidates: wideCount, cloudSyncSkipped: cloudSyncSkipped } : undefined
    },
    category, extTop, topDirs, topFiles: results.topFiles, junk,
    emptyDirSample: results.emptyDirs, timeBuckets, uncategorizedTop
  };
}
...
async function buildSuggestions(junkArr, emptySample, emptyCount) {
  ...
  if (removable.length > 0) s.push({ type: 'duplicates', title: '删除重复文件', risk: 'medium', estBytes: dupBytes, groups: removable.slice(0, 50), note: '保留路径最短者，其余移入回收站（范围：用户区 + 扫描根浅层）' });
  // 5) 大陈旧文件（中，非游戏库区）
...
function buildMarkdown(out, elapsedMs, lang) {
  const i18n = require('./i18n.js');
  const L2 = lang === 'zh' ? '' : '';
  const sm = out.summary || {};
  const L = [];
  L.push('# ' + (lang === 'zh' ? '磁盘扫描报告' : 'Disk Scan Report'));
  L.push('');
  L.push('> ' + (lang === 'zh' ? '生成时间' : 'Generated') + '：' + (out.generatedAt || sm.scannedAt || '—') + ' ｜ ' + (lang === 'zh' ? '耗时' : 'Elapsed') + '：' + (elapsedMs ? (elapsedMs / 1000).toFixed(1) + (lang === 'zh' ? ' 秒' : ' s') : '—') + ' ｜ ' + (lang === 'zh' ? '状态' : 'Status') + '：' + (sm.status || '—'));
  L.push('');
  L.push('## ' + (lang === 'zh' ? '概览' : 'Summary'));
  L.push('');
  L.push('| ' + (lang === 'zh' ? '项目' : 'Item') + ' | ' + (lang === 'zh' ? '数值' : 'Value') + ' |');
  L.push('|---|---|');
  L.push('| ' + (lang === 'zh' ? '扫描范围' : 'Scanned roots') + ' | ' + ((sm.roots || []).join(lang === 'zh' ? '、' : ', ') || '—') + ' |');
  L.push('| ' + (lang === 'zh' ? '总大小' : 'Total size') + ' | ' + fmtBytesMD(sm.totalBytes || 0) + ' |');
  L.push('| ' + (lang === 'zh' ? '文件数' : 'Files') + ' | ' + (sm.totalFiles || 0) + ' |');
  L.push('| ' + (lang === 'zh' ? '目录数' : 'Directories') + ' | ' + (sm.totalDirs || 0) + ' |');
  L.push('| ' + (lang === 'zh' ? '空目录' : 'Empty dirs') + ' | ' + (sm.emptyDirs || 0) + ' |');
  L.push('| ' + (lang === 'zh' ? '应用/游戏库' : 'App/game libraries') + ' | ' + fmtBytesMD(sm.appZoneBytes || 0) + '（' + (sm.appZoneFiles || 0) + (lang === 'zh' ? ' 个文件，仅统计不深度分析' : ' files, stats only') + '） |');
  const sk = sm.skipped || {};
  const skKeys = Object.keys(sk).filter(k2 => sk[k2]);
  if (skKeys.length > 0) L.push('| ' + (lang === 'zh' ? '跳过' : 'Skipped') + ' | ' + skKeys.map(k2 => k2 + ':' + sk[k2]).join(lang === 'zh' ? '，' : ', ') + ' |');
  L.push('');
  const cat = (out.category || []).slice(0, 15);
  const catRest = (out.category || []).slice(15).reduce((a, c) => a + (c.bytes || 0), 0);
  if (cat.length > 0) {
    L.push('## ' + (lang === 'zh' ? '类别统计' : 'Category breakdown'));
    L.push('');
    L.push('| ' + (lang === 'zh' ? '类别' : 'Category') + ' | ' + (lang === 'zh' ? '文件数' : 'Files') + ' | ' + (lang === 'zh' ? '大小' : 'Size') + ' | ' + (lang === 'zh' ? '占比' : 'Share') + ' |');
    L.push('|---|---|---|---|');
    ...
    if (catRest > 0) L.push('| ' + (lang === 'zh' ? '其他类别' : 'Others') + ' | — | ' + fmtBytesMD(catRest) + ' | — |');
    L.push('');
  }

  // T6：把规则缺口明确写出来。这不是"补充信息"，而是让分类对自己的诚实度可核查——
  // 若"其他"仍占大头，用户在这里能直接看到是哪几个扩展名造成的、各占多少，
  // 而不是只面对一个没人能处理的百分比。
  const uncat = (out.uncategorizedTop || []).filter(function (u) { return u.bytes > 0; });
  if (uncat.length > 0) {
    L.push('## ' + (lang === 'zh' ? '未分类占用 Top' : 'Uncategorized by size'));
    L.push('');
    L.push('> ' + (lang === 'zh'
      ? '这些扩展名没有任何分类规则覆盖，它们都计入上面的「其他」。列出这一段是为了让规则缺口自己暴露，而不是藏在一个总数里。'
      : 'These extensions match no category rule and are included in "Other" above. Listed so the gaps are visible rather than hidden inside a total.'));
    L.push('');
    L.push('| ' + (lang === 'zh' ? '扩展名' : 'Extension') + ' | ' + (lang === 'zh' ? '大小' : 'Size') + ' |');
    L.push('|---|---|');
    for (const u of uncat) L.push('| `' + u.ext + '` | ' + fmtBytesMD(u.bytes) + ' |');
    L.push('');
  }
  const tf = (out.topFiles || []).slice(0, 10);
  if (tf.length > 0) {
    L.push('## ' + (lang === 'zh' ? '大文件 Top 10' : 'Largest files (Top 10)'));
    L.push('');
    L.push('| # | ' + (lang === 'zh' ? '文件' : 'File') + ' | ' + (lang === 'zh' ? '大小' : 'Size') + ' |');
    L.push('|---|---|---|');
  ...
  if (td.length > 0) {
    L.push('## ' + (lang === 'zh' ? '大目录 Top 10' : 'Largest directories (Top 10)'));
    L.push('');
    L.push('| # | ' + (lang === 'zh' ? '目录' : 'Directory') + ' | ' + (lang === 'zh' ? '大小' : 'Size') + ' | ' + (lang === 'zh' ? '文件' : 'Files') + ' |');
    L.push('|---|---|---|---|');
  ...
  if (sugg.length > 0) {
    L.push('## ' + (lang === 'zh' ? '智能建议（' + sugg.length + ' 类）' : 'Smart suggestions (' + sugg.length + ')'));
    L.push('');
    sugg.forEach((s, i) => {
      L.push('### ' + (i + 1) + '. ' + (s.title || s.type) + (s.risk ? (lang === 'zh' ? '（风险：' + s.risk + '）' : ' (risk: ' + s.risk + ')') : ''));
      if (s.note) L.push('');
      if (s.note) L.push(s.note);
      if (s.estBytes != null) { L.push(''); L.push('- ' + (lang === 'zh' ? '涉及大小' : 'Estimated size') + '：' + fmtBytesMD(s.estBytes)); }
      if (s.type === 'organize-folders' && Array.isArray(s.items) && s.items.length > 0) {
        L.push('');
        L.push('| ' + (lang === 'zh' ? '目录' : 'Directory') + ' | ' + (lang === 'zh' ? '大小' : 'Size') + ' | ' + (lang === 'zh' ? '修改日期' : 'Modified') + ' | ' + (lang === 'zh' ? '类型' : 'Type') + ' | ' + (lang === 'zh' ? '去向/提示' : 'Target / note') + ' |');
        L.push('|---|---|---|---|---|');
        for (const it of s.items) {
          const kind = it.kind === 'program' ? (lang === 'zh' ? '⚠ 程序目录' : '⚠ program dir') : (lang === 'zh' ? '✅ 可整理' : '✅ movable');
          const dst = it.kind === 'program' ? (it.warn || (lang === 'zh' ? '仅提示，不移动' : 'info only')) : ('→ `' + (it.suggestDst || '') + '`');
        ...
        L.push('');
        L.push('| ' + (lang === 'zh' ? '保留' : 'Keep') + ' | ' + (lang === 'zh' ? '可删除数' : 'Removable') + ' | ' + (lang === 'zh' ? '每组大小' : 'Group size') + ' |');
        L.push('|---|---|---|');
...
async function main() {
  ...
  const lang = i18nDetect(args.indexOf('--lang') >= 0 ? (args[args.indexOf('--lang') + 1] || '') : '');
  ...
  if (modeOrganize) {
    ...
    // 评审 A3：此处曾自留一份 7 段 DANGER 名单（缺 windows.old/boot/efi/recovery/perflogs/
    // msocache/config.msi/$windows.~bt/~ws 共 9 段），且与实际闸门 guard.js 不联动。
    // 现统一走 guard，与 clean/organize/dedup/serve 完全一致。
    const DANGER = guard.PROTECTED_SEGMENTS;
    const moved = [], failed = [], duplicated = [];
    ...
    for (const item of plan) {
      ...
      // 用 guard.isProtectedPath 而非裸 indexOf：后者不含尾部匹配（`C:\Windows` 无尾分隔符时漏判），
      // 且 A4 的 `C:\整理区\..\boot\x` 形态在补全 16 段后也会被这里兜住（第二道防线）。
      if (guard.isProtectedPath(dl) || guard.isProtectedPath(sl)) { failed.push({ src: src, dst: dst, reason: '涉及系统目录' }); continue; }
      try {
        ...
        // v0.7.0（v7-7）：跨盘移动是"先 copy 再 unlink 源"。若 copy 成功而 unlink/rm 失败，
        // 会**留下两份副本**——而重试又被 COPYFILE_EXCL 卡成 EEXIST，最终用户只看到
        // `fail: EEXIST`，既不知道副本已经在目标盘上，也不知道源还在。
        // 这里显式识别该状态并说清楚：两份都在，请人工确认后删其一。
        let reason = (e && e.code) || (e && e.message) || String(e);
        try {
          if (await existsP(dst)) {
            let bothIntact = false;
            try {
              const sst = await fsp.stat(src);        // 源仍在？（unlink 失败的话它还在）
              const dst2 = await fsp.stat(dst);
              bothIntact = sst.isDirectory() ? dst2.isDirectory() : dst2.size === sst.size;
            } catch (e2) { bothIntact = false; }
            if (bothIntact) {
              reason = '已复制但源未删除（两份副本并存）：源 ' + src + ' / 目标 ' + dst +
                ' —— 请确认后删除其一（本工具未自行删除，以免误删）';
              duplicated.push({ src: src, dst: dst });
            }
          }
        } catch (e2) { /* 探测失败就退回原始原因 */ }
        failed.push({ src: src, dst: dst, reason: reason });
      }
    }
    let map = [];
    try { map = JSON.parse(fs.readFileSync(mapFile, 'utf8')); if (!Array.isArray(map)) map = []; } catch (e) { map = []; }
    // 仅当确有移动时才追加回滚记录（空批次不写，避免污染最后一批）
    // 评审 B3：改为原子写。旧实现直接 writeFileSync，进程在写入中途被杀就会留下
    // 半截 JSON —— 回滚读取时 JSON.parse 失败被 catch 成空数组，于是"静默变成没有记录"，
    // 已经移动过的文件永远回不来。现写临时文件再 rename（同目录 rename 在 NTFS 上原子替换）。
    if (moved.length > 0) {
      map.push({ ts: new Date().toISOString(), items: moved });
      try { writeFileAtomic(mapFile, JSON.stringify(map, null, 2)); } catch (e) { /* 记录失败但不中止 */ }
    }
    return { exitCode: 0, data: { ok: true, movedCount: moved.length, failedCount: failed.length, moved: moved.slice(0, 200), failed: failed.slice(0, 50), duplicatedCount: duplicated.length, duplicated: duplicated.slice(0, 50), mapFile: mapFile } };
  }
    ...
    if (mapFile2 && Array.isArray(r.fixed) && r.fixed.length > 0) {
      try {
        ...
        if (map.length > 0) {
          ...
          writeFileAtomic(mapFile2, JSON.stringify(map, null, 2));
        }
      } catch (e) { /* 记录失败但不中止 */ }
    }
  ...
  if (reportFile) {
    // T2：写入溯源。报告是全局单例、每次扫描直接覆盖，若不记下生成时间与范围，
    // 用户就无法判断手里这份是不是旧的——而 clean/organize 的候选路径恰恰取自"最近报告"。
    require('./report.js').stamp(out);
    ...
    try {
      const md = buildMarkdown(out, out.elapsedMs || 0, lang);
      const mdFile = reportFile.replace(/\.json$/i, '') + '.md';
      fs.writeFileSync(mdFile, md, 'utf8');
      out.mdFile = mdFile;
    } catch (e) { out.mdError = e && e.message ? e.message : String(e); }
    // T3：每次扫描都归档一份到报告历史（保留最近 N 份，实现 retention.reports）。
    // 归档失败**不中断扫描**——历史是附加价值，不该让主流程失败；但要如实上报，
    // 不能默默什么都没存却让人以为存了。
    try { archiveInfo = require('./report.js').archive(out); } catch (e) { archiveInfo = { ok: false, error: e && e.message ? e.message : String(e) }; }
  }
  const compact = {
    ok: true,
    reportFile: reportFile || null,
    archiveFile: archiveInfo && archiveInfo.ok ? archiveInfo.file : null,
    archiveError: archiveInfo && !archiveInfo.ok ? archiveInfo.error : undefined,
    ...
    // T6：未分类 Top（占用最大的未分类扩展名），让调用方（CLI/MCP/GUI）也能显示规则缺口
    uncategorizedTop: out.uncategorizedTop || [],
