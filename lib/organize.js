...
// 评审 A5：此处曾自写一份带边界 bug 的前缀匹配（lp.indexOf(low(r)) === 0），
// guard.js 早已修好同类问题（C:\Users 会命中 C:\UsersOther\）。统一走 guard。
function inScanRoots(lp, roots) {
  return guard.inRoots(lp, roots);
}
// 评审 A3：此处曾自留一份 7 段 SYS_PREFIX（缺 windows.old/boot/efi/recovery 等 9 段），
// 且无 guard 的尾部匹配 → 同一个 C:\Users\me\windows 在 clean 侧被拒、在此放行。
// 现统一 require('./guard.js')，与 clean/dedup/serve 完全一致。
function isSafeZone(lp) {
  return !guard.isProtectedPath(lp);
}
...
async function apply(items, opts) {
  ...
  for (const it of items) {
    ...
    // 评审 A4：目标此前只做前缀正则，`C:\整理区\..\boot\x` 能通过，
    // 而 OS 会把它解析成 `C:\boot\x` —— 管理员会话下真的会把文件移进受保护目录。
    // 现在：先归一化、拒绝任何 . / .. 段、再要求归一化后的父目录就是 <盘>:\整理区。
    const dstRes = guard.checkOrganizeDest(it.dst);
    if (!dstRes.ok) return { ok: false, error: dstRes.error + '：' + (it.dst || '') };
  ...
  appendAudit({ ts: new Date().toISOString(), type: 'organize', action: 'apply', paths: items.map(function(it) { return it.src }), freedBytes: 0, executed: rdata.movedCount || 0, result: (rdata.duplicatedCount ? 'partial' : 'ok'), detail: JSON.stringify({ failed: rdata.failed || [], duplicated: rdata.duplicated || [], shortcutFixed: shortcutFixed, shortcutError: shortcutError }).slice(0, 500) });
  return { ok: true, movedCount: rdata.movedCount, failedCount: rdata.failedCount, failed: rdata.failed, duplicatedCount: rdata.duplicatedCount || 0, duplicated: rdata.duplicated, shortcutFixed: shortcutFixed, shortcutError: shortcutError, note: '已整理 ' + rdata.movedCount + '/' + items.length + ' 项' + (shortcutFixed > 0 ? '，自动重写快捷方式 ' + shortcutFixed + ' 个' : '') + (rdata.duplicatedCount ? '，另有 ' + rdata.duplicatedCount + ' 项出现双份副本（见 duplicated）' : '') + '，可回滚' };
}
