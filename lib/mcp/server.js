...
function createServer(options) {
  ...
  function start() {
    // 编码（v0.7.0 v7-10，已排查、未复现，故不改）：
    // 有评审怀疑"未显式 setEncoding('utf8') 会让 GBK 管道下的中文乱码"。
    // 实测（UTF-8 客户端，符合 MCP 规范）：tools/list 返回的中文描述无 U+FFFD，
    // 且含中文的 disk_config 参数能原样往返 —— readline 对无编码的流默认按 UTF-8 解码，
    // 故这里无需再显式 setEncoding。若将来真出现 GBK 客户端乱码，那是客户端违反
    // MCP 的 UTF-8 约定（协议规定 stdio 为 UTF-8），应在客户端侧修，不要在这里做二次解码。
