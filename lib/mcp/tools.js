...
// 8.3 短名（ADMINI~1 vs Administrator）、junction/symlink 目标、映射盘、
// 尾部分隔符、大小写、`.`/`..`。纯小写字符串比较在任一情况下都会失配，
// 结果是"刚清理完却查不到可恢复项"（实测：短名路径清理后 toolMatched=0）。
//
// 关键点：被清理的目录已经不在磁盘上了，直接对它 realpath 必然失败，
// 两边都会保留各自拼写而继续失配。所以要**向上找到最深的仍存在的祖先**做
// realpath，再把缺失的尾段拼回去 —— 这样短名与长名都能归一到同一种拼写。
function canonKey(p) {
  const fs = require('fs');
