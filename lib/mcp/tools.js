...
function tools() {
  return [
    ...
    {
      name: 'disk_config',
      description: '读取或修改规则配置（~/.disk-clean/config.json）：清理/整理阈值（散落目录与陈旧文件的大小与天数、重复文件最小字节）、exclude 排除前缀、保留策略（auditLines/reports）、自定义垃圾规则与整理映射。cmd=get 读当前配置（含默认值合并结果）；cmd=set 改一项；cmd=reset 恢复默认。注意：配置只能收窄而不能放宽安全边界——系统目录与 OneDrive 始终由 lib/guard.js 拒绝，不受配置影响。',
