...
// T3：报告历史目录的唯一来源是 lib/audit.js。此前这里自己算了一遍同样的路径，
// 而"同一个目录两处定义"正是不一致反复发生的来源。保留本函数只为兼容既有调用方。
function reportsDir() { return require('./audit.js').reportsDir() }
...
// ---------- run ----------
// 由任务触发：读配置 → scan → 报告（归档由引擎统一做，见 lib/report.js 的 archive）
async function run(name, engineRun) {
  ...
  const argv = ['--roots', cfg.roots.join(';'), '--suggest'];
  ...
  try {
    ...
    const d = res.data || {};
    // T3：这里不再自己指定报告路径写进 reports/。那样做会与引擎的统一归档产生
    // 两份同一时刻的报告（一份明文、一份 gz），历史列表里就会出现重复条目。
    // 现在定时扫描走默认报告路径，归档交给引擎；归档位置如实回传。
    return {
      ok: true,
      reportFile: d.reportFile || null,
      mdFile: d.mdFile || null,
      archiveFile: d.archiveFile || null,
      summary: d.summary,
    };
  } catch (e) {
