...
> 主题：**让它对最真实的磁盘有用**。这一版集中修"在真实使用中才暴露"的问题：
> 分类对游戏/模型盘失效、错误消息是死路、GUI 每次启动弹 UAC、报告只能看最近一份。
> 详见 `docs/RELEASE_NOTES-v0.7.2.md` 与 `docs/PRODUCT-REVIEW.md`（P0–P3 已全部完成）。

### Added
- **`disk-clean doctor`**：环境自检 + 首次引导。检查 Node 版本、状态目录可写、管理员权限、
  最近报告是否过期、回收站是否可访问，**逐项给下一步命令**，收尾给"从这里开始"的三步。
  此前新用户只能从文档里推断"要先 scan"。
- **`report --html`**：导出**自包含单文件** HTML（样式内联、零外部引用），双击可开、转发后样子不变。
  每一处插值都转义——报告字段来自被扫描的磁盘，未转义的类别名会在打开者浏览器里执行。
- **报告的"未分类占用 Top"**：新增 `uncategorizedTop` 字段与 Markdown 段落。没有它，
  规则缺口只会表现为一个没人能处理的「其他 XX%」；有了它，"pak 296.88 GB 未分类"直接指向该补哪条规则。
- **`docs/CAPABILITY-MATRIX.md`**：24 条能力 × CLI/MCP/GUI 三面，含不对称的**理由**。
  由 `test/capability-matrix.js` **双向**守门：矩阵写的入口必须存在，实现里的入口必须都在矩阵里。
- **GUI 报告历史端点**：`GET /api/reports`（清单）与 `?file=<name>`（单份归档）。
- `lib/suggest.js`（"最接近的匹配"）、`lib/reporthtml.js`（HTML 生成）。

### Changed
- **分类重建（本次最大的一项）**：真实盘上「其他」从 **76.6% 降到 11.8%**。
  新增「游戏资产 / AI 模型 / 开发产物 / 移动安装包」四类与 `models`、`hf_cache`、`wegameapps`、
  `backup` 等目录规则；分类改为**扩展名优先、目录语境兜底**（`.bin`/`.dat` 这类扩展名本身
  不携带用途信息）。无扩展名单独成类（真实盘 31.87 GB，排第 4）。
- **GUI 不再恒定提权**：`app.manifest` 从 `requireAdministrator` 改为 `asInvoker`，
  启动不再弹 UAC。真正需要管理员的三个端点（MFT 直读 / SMART / 配额）在缺权限时返回明确提示。
- **CLI 多范围扫描要求确认**：先列出各范围与其已用容量，再要求 `--yes`（与 GUI 的范围弹窗对等）。
  单范围不要求——命令行本身就是范围。
- **进度反馈**：非 TTY（管道 / CI 日志）下不再用 `\r` 覆盖同一行（那会把日志刷成一堆半行），
  改为每 3 秒一行。**不假称 ETA**：扫描前总量未知，画百分比是编造；改为已处理量 + 已用时间 + 吞吐速率。
- **错误消息不再是死路**：`clean` 缺类型会列出可选值并给示例（此前是 `未知清理类型: undefined`）；
  `junk` → `是否想用 junk-temp？`（前缀匹配）；`emty-dirs` → `empty-dirs`（编辑距离）；
  `scna` → `是否想用 scan？`。**无关输入不给建议**——乱猜比不说更让人困惑。
- `report` 头部现在同时显示生成时间、扫描范围与**版本**。
- GUI 启动时加载最近报告（此前端点存在但从不调用，打开界面看不到上次结果）。

### Fixed
- **CLI 的 `fail()` 静默丢弃 hint**：只接受一个参数，所有调用方传的第二个参数都被丢掉；
  而 MCP 侧的 `fail()` 一直支持。拒绝必须带下一步，现在两侧一致。
- **`clean` 的类型校验顺序**：以前排在"读报告"之后，于是全新环境里拼错类型会先收到
  "未找到扫描报告"，用户跑去扫一遍才发现真正的问题是类型写错了。类型是否合法不需要报告即可判定。
- `/api/reports?file=` 的目录穿越防护（只接受历史目录内的文件名）。

### Docs
- `ROADMAP.md` 补 `Phase 17`(v0.7.0) / `Phase 18`(v0.7.1)；修正 `Phase 11` 的错误验收
  （原文写着一行从未实现的能力）。
- `PRODUCT-REVIEW.md` 的 P0–P3 **全部标记完成**，并保留两处事后修正（T5 的错误诊断、版本标号）。
## [0.7.1] - 2026-09-12
