...
**验收（2026-09-12 修正）**：`--lang en|zh` 切换的是**报告的结构性标签**（Markdown 标题、
字段名、"生成时间/耗时/状态"那一行）。**CLI 自身的控制台输出与建议标题始终是中文**。

> ⚠️ 本项原验收写的是"`--lang en` 输出全英文报告与帮助"，与实现不符：帮助与建议标题从未翻译。
> v0.7.1 按"如实说明"处理——英文 README 顶部明确写出"CLI 输出为中文、`--lang` 只影响报告结构标签"，
> 不再让人以为存在英文界面。**验收条件写宽了，就会长期显示为"已完成"却与事实不符**，
> 这正是本仓库反复出现的"文档比实现更乐观"；可机器判定的部分现已由
> `test/docs-consistency.js` 守门，但"这句话是否夸大"仍需人来判断。
...
| Phase 16 | 2026-09-11 | v0.6.0（**安全加固**）：修复独立锐评发现的 1 个 P0 + 6 项安全/正确性缺陷 —— 硬链接合并拒绝 `approx` 抽样组（P0，曾致不可逆内容破坏）、硬链接过安全闸门 + 扫描根校验、保护名单 4 份副本收敛为真单源、整理目标防路径穿越、`dedup-map` schema 统一、回收站 `canonKey` 单源、`partial` 部分成功上报、回滚映射原子写、畸形 URL 兜底、`checksums.txt` 发布一致性（G53/G54）；新增 `test/safety-gates.js`，测试 **11** 套件全绿 |
| Phase 17 | 2026-09-12 | v0.7.0（**质量周**）：清除"工具在骗你"的三类问题 —— 假承诺（`config.blacklist` 从未被读取却被工具描述承诺，已删）、口径不一（`retention.auditLines` 配置写了没人读，现惰性生效）、夸大数字（锐评原估"上千重复文件需 30–50 分钟"实测仅 2.4 分钟，据此重排优先级）；另修 CLI 缺 `stale-large`、`dedup rollback` 统一要求 `--yes`、跨盘"双份副本"显隐式上报；`drives` 抽为单源；硬链接合并由 PowerShell 改 `fs.linkSync`（144ms/个 → 0.66ms，**218x**）、dedup 的 `stat` 并发化（1 万文件 292→71ms）；测试 11→**12** 套件 |
| Phase 18 | 2026-09-12 | v0.7.1（**可信数据 + 脚本级门禁**）：①**流程** S3–S10 全部从"文档约定"升级为"脚本级拦截"——发布审批绑定**每个产物的 sha256**、制品指纹断言 build==source、文档一致性闸门（含"测试写了却没注册"与死配置）、声明式发布清单、变更分级、CHANGELOG/发版指南生成器、完成前验证铁律；**CI 不再在 push tag 时自动发 Release**（那等于绕过审批）②**产品** T1–T5——测试不再写用户真实 `~/.disk-clean`（两层守卫：静态 + 内容哈希快照）、报告溯源（生成时间/范围进 CLI+MCP+GUI，>24h 提示）与报告历史（实现 `retention.reports`，gzip 归档）、删除最后两个死配置键、CLI 建议显示修复；`--lang en` 改为诚实说明；测试 12→**17** 套件（+2 项 runner 自检 = **19 项检查**）；已发布 GitHub Release（6 资产）+ npm（`latest=0.7.1`）|

---

## 18. 后续方向（v0.8 起）

Phase 0–18 是**已完成的能力建设史**。后续整改方向以 `docs/PRODUCT-REVIEW.md` 为准
（那里有成体系的优先级、真实盘实测数字与验收条件）：

| 优先级 | 项 | 目标 / 验收 |
|---|---|---|
| P2 | **T6 分类重建** | 真实盘上最大分类是"其他"（**76.6%**，Top 22 扩展名里 12 个未分类）→ 目标 **< 30%**，并输出"未分类 Top N 扩展名" |
| P2 | **T7 能力矩阵** | 能力下沉 `lib/`，三个协议壳（GUI+CLI 共用同一 exe / MCP）只做转换；用测试守住三处一致 |
| P2 | **T8 错误消息与 doctor** | 缺参数/拼错给出下一步；`disk-clean doctor` 自检（E1–E5） |
| P3 | **T9–T13** | 进度 ETA + 非 TTY 降级、CLI 扫描范围确认、**GUI 去恒定提权**、HTML 报告导出 |

> **为什么不再往本文件追加 Phase**：Phase 列表记录"做完了什么"，而待办会随实测数字变化
> （T6 的目标就是从真实扫描结果里量出来的）。放在 PRODUCT-REVIEW 里可以带着证据一起更新，
> 也避免"同一件事两处写、然后各自漂移"——本仓库已经因为这类重复吃过多次亏。
