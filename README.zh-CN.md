...
**在真实磁盘上终于有用了。** 在一台 588 GB、塞满游戏资产与 AI 模型权重的真实磁盘上，
分类以前给出的答案是「其他 76.6%」——最大的类别叫"未分类"。现在是 **11.8%**，
而且报告会列出仍然没有规则覆盖的大扩展名，让缺口自己指向自己。

- **分类按真实占用重建**：游戏资产（`.pak` 一个就 296.88 GB）、AI 模型权重、编译产物、
  移动安装包，并为 `.bin` 这类本身不携带含义的扩展名补上**目录语境**判定。
- **GUI 不再每次启动要管理员权限**（扫描/清理/整理/去重本来就不需要）；
  真正需要的三个功能会明确说出来，而不是给你一句晦涩的失败。
- **错误消息会指明方向**：`clean` 缺类型会列出可选值，`junk` 建议 `junk-temp`，
  `scna` 建议 `scan`；完全不像的输入则不给建议。
- **`disk-clean doctor`**（首次使用引导）；**`report --html`**（可分享的单文件报告）；
  **`scan C:\ D:\`** 现在会先确认范围再遍历。

详见 [docs/RELEASE_NOTES-v0.7.2.md](docs/RELEASE_NOTES-v0.7.2.md)。
## v0.7.1 更新内容

**可信数据**。补丁版——不加新能力，只有两批修复。

- **测试不再动你的真实数据**：当时十五个套件里有十二个曾直接读写你真实的 `~/.disk-clean/`——
  扫描报告被覆盖成测试临时树的结果、审计日志被污染、测试文件被丢进你的回收站。
  现已全部隔离，并加两层守卫（静态：新增套件漏隔离则整体失败；运行时：跑测试前后对真实目录做内容哈希比对）。
- **报告有了出处和历史**：生成时间与扫描范围现在会出现在 CLI、MCP 响应与 GUI 界面上，
  超过 24 小时会提示重新扫描。每次扫描还会自动归档（gzip）到 `~/.disk-clean/reports/`，
  按 `retention.reports` 保留最近 N 份——这个配置项存在了好几个版本却一直没有代码读它。
  用 `disk-clean report --history` 查看。
- **`disk-clean build-info`**：输出版本、commit，以及构建时源码树是否干净。
  发布页的 `checksums.txt` 带同一个 commit，因此下载到的 exe 可以对照源码修订核对，而不只是看文件名。
- **建议行修复**：此前输出 `[junk-temp] junk-temp`，因为 CLI 读了一个不存在的字段（真名是 `title`）。
  本 README 记录的示例输出一直是对的，错的是代码。
- **删除两个失效配置项**：引擎从未读取它们，却在 MCP 工具描述里被当作已有功能承诺。
  （具体是哪两个、为什么删，见 CHANGELOG——本文件说明工具**能做什么**，因此不列出已删除的能力。）

详见 [docs/RELEASE_NOTES-v0.7.1.md](docs/RELEASE_NOTES-v0.7.1.md)。
...
npm test          # 全量测试（19 个套件，含 MCP 协议 / 规则完整性 / 安全闸门 / 文档一致性回归）
