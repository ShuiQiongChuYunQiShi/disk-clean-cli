...
// 当前没有未修复的欠债。留空数组不是"检查被关掉了"：下面的循环仍然会校验每一项
// （将来新增欠债时在这里登记即可），而且额外断言 §3 必须**保留已修复记录**——
// 修好一项就从欠债表移走，但不能连"它曾经存在"一起删掉，否则同一个不对称会被重新引入。
const DEBT_PATTERNS = [];
for (const d of DEBT_PATTERNS) {
  assert(d.match.test(debt),
    '§3 里登记的「' + d.key + '」不见了。若确实已修复，请把该行移入"已修复"小节；' +
    '若还没修，请不要把它从文档里抹掉——那正是"大家以为三处一致"的来源。');
}
assert(/已修复/.test(debt),
  '§3 必须保留"已修复"记录：欠债修好后从待办表移走是对的，但连它曾经存在也删掉，' +
  '同一个能力不对称就会在下次重构时被重新引入。');
