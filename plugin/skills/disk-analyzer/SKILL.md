...
> 说明：本静态形态提供**模型侧工具 + 文本报告**；交互式**图表面板**由动态插件版本提供
> （在 cordis 会话中用 cordis_define 安装 host.js + client.js），面板为 **5 个 Tab 导航**：
> 概览 / 清理中心（一键清理临时·空目录·陈旧大文件·回收站）/ 重复文件（扫描期查重 +
> 深度全盘查重 + 硬链接合并可回滚）/ 一键整理（plan→勾选→apply→rollback）/ 磁盘健康
> （SMART 全字段 + 物理盘↔卷映射 + 总评）。两者可并存。本预设与 `cordis`（创造模式）预设共存，互不冲突。
