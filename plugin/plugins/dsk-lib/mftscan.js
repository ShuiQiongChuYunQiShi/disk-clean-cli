...
function scan(drive, opts) {
  opts = opts || {};
  if (!opts.categoryMap) {
    try { opts.categoryMap = require('./ext-cat.js').EXT_CAT; } catch (e) { /* 引擎不可用则不分类 */ }
  }
  const t0 = Date.now();
  let vol;
  ...
  function readRun(physBase, runRecs) {
    ...
    while (i < runRecs) {
      ...
      for (let j = 0; j + REC_SIZE <= usable; j += REC_SIZE) {
        ...
        if (parsed) {
          // 卷容量兜底：size 不可能超过卷大小（稀疏文件逻辑大小不计入磁盘占用）
          if (parsed.size > vol.volumeBytes) parsed.size = 0;
  ...
  const dirsFull = Object.keys(dirBytes).map(function(d) {
    return { path: drive + '\\' + d, bytes: dirBytes[d], files: dirFiles[d] || 0 };
